...
# Read in data
data <- read.csv("data/inla_input/data_dengue_cru_cell.csv") %>% 
  subset(Year > 2000 & Year < 2015) %>% 
  dplyr::select(-X) %>% 
  mutate(Date = make_date(Year, Month)) %>% 
  mutate(timestep = rep(1:168, each = 2)) 

avg <- NULL
for (j in unique(data$cell)){
  df <- subset(data, data$cell == j)
...
for (i in unique(data$cell)){
  ...
  save(dengue_estimates, file = "models/dengue/estimates_cru_cell.RData")
...
# Read in data
data <- read.csv("data/inla_input/data_malaria_cru_cell.csv") %>% 
  subset(Year > 1988 & Year < 2016) %>% 
  dplyr::select(-X) %>% 
  mutate(Date = make_date(Year, Month)) %>% 
  mutate(timestep = rep(1:324, each = 2)) 
...
for (i in unique(data$cell)){
  ...
  # Climate effects
  prcp  <- scale(df$prcp, center = TRUE, scale = TRUE)[,1]             
  tmean <- scale(df$tmean, center = TRUE, scale = TRUE)[,1] 
  ...
  malaria_estimates <- rbind(malaria_estimates, 
                             mod$summary.fixed %>% 
                               ...
                               dplyr::rename(lci = `0.025quant`,
                                             uci = `0.975quant`)) %>%
    tibble::remove_rownames()
  save(malaria_estimates, file = "models/malaria/estimates_cru_cell.RData")
